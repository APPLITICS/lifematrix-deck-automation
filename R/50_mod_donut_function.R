...
generate_donut_slide <- function(
    ...
    ggplot(df, aes(x = 2, y = percent, fill = .data[[category_var]])) +
      ...
      theme(
        plot.title = element_text(
          size = 24, face = "bold", color = "white",
          hjust = 0.5, vjust = 0.05, margin = margin(b =20)
          ),
  ...
  # combine all donut components side by side
  donut_row <- cowplot::plot_grid(
    plotlist = components,
    nrow = 1,
    rel_widths = rep(c(1, 0.01), length.out = length(components))[1:length(components)]
  )
  
  # combine donuts + legend vertically
  ...
  # add background theme (applied once)
